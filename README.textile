...
| sql_prefix_file | if passed, the contents of this file will be appended to every migration run. Useful for setting deployment specific variables if migrations are used across many databases | - | any filepath
| database_engine_use_cli | if True, will use a CLI interface instead of the native drivers. Occasionally useful for specialized scripting unsupported by drivers. Currently only implemented for Oracle's sqlplus client | False | True
